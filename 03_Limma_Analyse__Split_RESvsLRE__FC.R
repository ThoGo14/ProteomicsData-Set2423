...
condition <- ProteomicsData__Metadata[c("Timepoint", "MyoID", "Rating_FC"), ]
...
for (i in TP_Comparisson) {
  ...
  a_group = "RES"
  a = unique(condition["MyoID", grep(a_group, condition["Rating_FC", ])])
  ...
  b_group = "LRE"
  b = unique(condition["MyoID", grep(b_group, condition["Rating_FC", ])])
  b_subject = NULL
  for(x in b){
...
for (i in TP_Comparisson) {
  ...
  # PairedSubjects <- Subjects_list[["cols"]][[i]]
  Expression <- ProteomicsDataNRE2[[paste0("ProteomicsData__", str_remove(i, "_FC"))]]
  ...
  xlsx::write.xlsx2(tt, paste("01_Limma analysis Tables/tt.", i, "_LRE-RES.xlsx", sep = ""))
...
for (TP in TP_Comparisson){
  data = ProteomicsDataNRE2[[paste0("ProteomicsData__", str_remove(TP, "_FC"))]]
...
save(final_data, file = "03_Limma_Analyse__Split_RESvsLRE__FC.RData")
...
saveWorkbook(wb, file = "03_Limma_Analyse__Split_RESvsLRE__FC.xlsx", overwrite = TRUE)
