...
PVal_cols <- colnames(LimmaAnalysisTable)[grep("pValue", colnames(LimmaAnalysisTable))]
# PVal_cols <- PVal_cols[!str_detect(PVal_cols, "adj.pValue")]
PVal_cols <- PVal_cols[-grep("FoldChange", PVal_cols)]
...
for (i in PVal_cols) {
  Uniprot_ID <- LimmaAnalysisTable[LimmaAnalysisTable[, i] < 0.05, "Uniprot_ID"]
  ...
  if (!is.null(gostres)) {
    ...
    write.csv(Results, file = paste0("10_GO Analysis Pictures - LimmaTable/", "gProfiler__", i, ".csv"))
    ...
    ggsave(paste0("10_GO Analysis Pictures - LimmaTable/", "gProfiler__", i, ".png"), dpi = 300, units = "cm", 
           width = 24, height = 15)
  }
...
for (i in PVal_cols) {
  ...
  for (Direction in c("positiveFoldChange", "negativeFoldChange")) {
    ...
    if(Direction == "positiveFoldChange"){
      Uniprot_ID <- LimmaAnalysisTable[LimmaAnalysisTable[, i] < 0.05 &
                                       LimmaAnalysisTable[, paste0(TP, "__Ratio")] > 1,
                                     "Uniprot_ID"]
    ...
    if (!is.null(gostres)) {
      ...
      write.csv(Results, file = paste0("10_GO Analysis Pictures - LimmaTable/", 
                                       "gProfiler__", i, ifelse(Direction == "positiveFoldChange",
      ...
      ggsave(paste0("10_GO Analysis Pictures - LimmaTable/", 
                    "gProfiler__", i, ifelse(Direction == "positiveFoldChange",
                                             "__positiveFoldChange",
