...
for (TP in names(KorrelationTable_Results_List)) {
  ...
  AnalysisTable <- KorrelationTable_Results_List[[TP]]
  
  KlinischeParameter <- unique(colnames(AnalysisTable)[(grep("Correlation__Analysis", colnames(AnalysisTable))+1L):
                                                         (grep("Gene Annotations", colnames(AnalysisTable))-1L)] %>% 
                                 sapply(function (x) unlist(str_split(x, "__"))[1]))
  for (i in KlinischeParameter) {
    
    
    Uniprot_ID <- AnalysisTable[AnalysisTable[, paste0(i, "__pValue")] < 0.05, "Uniprot_ID"]
    
    if (is_empty(Uniprot_ID)) {
      next
    }
    ...
    if (!is.null(gostres)) {
      ...
      write.csv(Results, file = paste0("15_GO Analyse Pictures - KlinischeDaten/", "gProfiler__", TP, "__", i, ".csv"))
      ...
      ggsave(paste0("15_GO Analyse Pictures - KlinischeDaten/", "gProfiler__", TP, "__", i, ".png"), 
             dpi = 300, units = "cm", width = 24, height = 15)
    }
    
    print(paste0(format(Sys.time(), "%x %T"), "  Finished: ", TP, " ", i))
  }
}
...
for (TP in names(KorrelationTable_Results_List)) {
  ...
  for (i in KlinischeParameter) {
    ...
    for (Direction in c("positiveFoldChange", "negativeFoldChange")) {
      
  
      if(Direction == "positiveFoldChange"){
        Uniprot_ID <- AnalysisTable[AnalysisTable[, paste0(i, "__pValue")] < 0.05 &
                                         AnalysisTable[, paste0(i, "__pearsonR")] > 0,
                                       "Uniprot_ID"]
      } else if(Direction == "negativeFoldChange"){
        Uniprot_ID <- AnalysisTable[AnalysisTable[, paste0(i, "__pValue")] < 0.05 &
                                         AnalysisTable[, paste0(i, "__pearsonR")] < 0,
                                       "Uniprot_ID"]
      }
      ...
      if (!is.null(gostres)) {
        ...
        write.csv(Results, file = paste0("15_GO Analyse Pictures - KlinischeDaten/", 
                                         "gProfiler__", TP, "__", i, ifelse(Direction == "positiveFoldChange",
                                                                            "__positiveFoldChange",
                                                                            "__negativeFoldChange"),
                                         ".csv"))
        ...
        ggsave(paste0("15_GO Analyse Pictures - KlinischeDaten/", 
                      "gProfiler__", TP, "__", i, ifelse(Direction == "positiveFoldChange",
                                                         "__positiveFoldChange",
                                                         "__negativeFoldChange"), ".png"), 
               dpi = 300, units = "cm", width = 24, height = 15)
      }
    }
    print(paste0(format(Sys.time(), "%x %T"), "  Finished: ", TP, " ", i))
